...
6.  **备注**：当前版本下，如果你使用**手机**给你的 Bot 发送消息，在消息输入过程中会捕抓到“对方正在输入”这一状态，导致主动消息的任务创建。这是一个近乎无害的特性，你可以利用这个特性在不发送消息的情况下创建一个主动消息任务。以防你有疑问（例如疑惑为什么没给 Bot 发消息就触发了主动消息），特此说明。
...
- **`最大未回复次数上限`**: 当 Bot 连续发送 N 次主动消息你都未回复后，将暂停主动消息，直到你下次回复。填0则不限制。
...
备注：本段描述的最近更新时间：2025/10/27，适用于v0.9.7
